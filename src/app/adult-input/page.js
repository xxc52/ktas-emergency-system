...
export default function AdultInput() {
  ...
  const [selectedFirstConsiderations, setSelectedFirstConsiderations] = useState([]);
  const [selectedSecondConsiderations, setSelectedSecondConsiderations] = useState([]);
  ...
  useEffect(() => {
    if (selectedCategory && ktasData.length > 0) {
      ...
      const allFirstConsiderations = getFirstConsiderationsByCategory(ktasData, selectedCategory);
      const allSecondConsiderations = getSecondConsiderationsByCategory(ktasData, selectedCategory);
  ...
  }, [selectedCategory, selectedFirstConsiderations, selectedSecondConsiderations, ktasData]);
  ...
  }, [selectedCategory, selectedDiseases, selectedSecondConsiderations, ktasData]);
  ...
  }, [selectedCategory, selectedDiseases, selectedFirstConsiderations, ktasData]);
  ...
  const handleNext = () => {
    if (selectedDiseases.length > 0 && (selectedFirstConsiderations.length > 0 || selectedSecondConsiderations.length > 0)) {
      ...
      const primarySecondConsideration = selectedSecondConsiderations[0] || null;
      
      const ktasLevel = getKtasLevel(ktasData, primaryDisease, primaryFirstConsideration, primarySecondConsideration);
  ...
  const removeFirstConsideration = useCallback((consideration) => {
    setSelectedFirstConsiderations(prev => prev.filter(c => c !== consideration));
  }, []);

  const removeSecondConsideration = useCallback((consideration) => {
    setSelectedSecondConsiderations(prev => prev.filter(c => c !== consideration));
  }, []);
  ...
  }, [firstConsiderations, selectedFirstConsiderations, firstConsiderationCompatibility]);
  ...
  }, [secondConsiderations, selectedSecondConsiderations, secondConsiderationCompatibility]);

  const handleBack = () => {
    router.push('/age-selection');
  };

  const isNextEnabled = selectedCategory && selectedDiseases.length > 0 && (selectedFirstConsiderations.length > 0 || selectedSecondConsiderations.length > 0);
    ...
    <div className="container">
      <div className="header">
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          {(selectedDiseases.length > 0 || selectedFirstConsiderations.length > 0 || selectedSecondConsiderations.length > 0) && (
      ...
      <div className="content">
        ...
        <div className="category-nav">
          {categories.map((category) => (
            <button
              key={category}
              className={`category-nav-button ${selectedCategory === category ? 'selected' : ''}`}
          ...
          <div className="ktas-container">
            ...
            <div className="ktas-section">
              ...
              {sortedFirstConsiderations.map((consideration) => {
                const isSelected = selectedFirstConsiderations.includes(consideration);
                const isCompatible = firstConsiderationCompatibility[consideration];
                ...
                return (
                  <button
                    ...
                    onClick={() => handleFirstConsiderationSelect(consideration)}
                  >
                    {consideration}
                  </button>
                );
            ...
            <div className="ktas-section">
              ...
              {sortedSecondConsiderations.map((consideration) => {
                const isSelected = selectedSecondConsiderations.includes(consideration);
                const isCompatible = secondConsiderationCompatibility[consideration];
                ...
                return (
                  <button
                    ...
                    onClick={() => handleSecondConsiderationSelect(consideration)}
                  >
                    {consideration}
                  </button>
                );
